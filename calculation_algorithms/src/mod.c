...
void __BIGINT_MONT_REDC__(const bigInt *t, mont_ctx ctx, bigInt *rem) {
    uint64_t m = t->limbs[0] * ctx.nprime, carry = 0;
    bigInt tmp_t; __BIGINT_INTERNAL_LINIT__(&tmp_t, 2*ctx.k);
    __BIGINT_INTERNAL_ENSCAP__(rem, ctx.k);
    for (size_t i = 0; i < ctx.k; ++i) {
        uint64_t lo, hi;
        lo = __MUL_UI64__(ctx.n->limbs[i], m, &hi);
        tmp_t.limbs[i] = __ADD_UI64__(t->limbs[i], lo + carry, &carry);
        carry += hi;
    } tmp_t.limbs[ctx.k] += carry;
    // Right Limb Shift by 1  --  t +>> 1 (+>> or +<< means LIMB SHIFT)
    for (size_t j = 0; j < 2*ctx.k - 1; ++j) tmp_t.limbs[j] = t->limbs[j+1];
    if (__BIGINT_INTERNAL_COMP__(&tmp_t, ctx.n) > 0) __BIGINT_INTERNAL_SUB__(&tmp_t, ctx.n);
